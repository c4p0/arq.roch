document.addEventListener('DOMContentLoaded', function() {
    ...
    function showSection(id) {
        ...
        // Encontrar el índice del botón objetivo
        const targetIndex = Array.from(navButtons).indexOf(targetButton);

        // Calcular desplazamiento solo para los botones inferiores
        let moveDistance = 0;
        ...
        if (lowerButtons.length > 0) {
            ...
            const lastButtonRect = lastButton.getBoundingClientRect();
            const spaceToBottom = viewportHeight - lastButtonRect.bottom - 40;
            moveDistance = Math.max(0, -spaceToBottom);
        }

        // Aplica el desplazamiento solo a los botones inferiores
        Array.from(buttonBlock.children).forEach(btn => {
            const navIdx = Array.from(navButtons).indexOf(btn);
            if (navIdx > targetIndex && moveDistance > 0) {
                btn.style.transform = `translateY(${moveDistance}px)`;
