document.addEventListener('DOMContentLoaded', function() {
    ...
    function showSection(id) {
        ...
        // --- LÓGICA CLAVE: MOVER SOLO EL BLOQUE DE BOTONES SELECCIONADO E INFERIORES ---
        // Solo aplica a botones de .button-block
        const btnIdx = Array.from(blockButtons).indexOf(targetButton);
        if (btnIdx !== -1 && btnIdx < blockButtons.length - 1) {
            // Selecciona el bloque de botones a mover (el seleccionado y los inferiores)
            const buttonsToMove = Array.from(blockButtons).slice(btnIdx);
            ...
            // Mueve solo el grupo seleccionado+inferiores
            ...
            // Los superiores no se mueven
            Array.from(blockButtons).slice(0, btnIdx).forEach(btn => {
                btn.style.transform = '';
                btn.style.transition = '';
            });
        } else {
            // Si es el último botón o un navButton, resetea todos
